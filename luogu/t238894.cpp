...
第一行四个正整数 $n,m,y,x$，含义如题面所示，**请注意意先 y 后 x**。
第二行一个长度为 $m$ 的字符串，表示珂朵莉的移动序列。

## 输出格式

共$n$ 行，每行 $n$ 个数，第 $i$ 行的第 $j$ 个数表示坐标 $(n-i+1,j)$ 上的灰的数量
...
珂朵莉的移动路径为 $(1,1)\rightarrow(1,2)\rightarrow(2,2)\rightarrow(2,3)$，共 $4$ 分钟。
...
**注意最后珂朵莉移动到了 $(2,3)$，但是时间只有 $4$ 秒，所以实际上不会对 $(2,3)$ 造成影响。初始点不一定在 $(1,1)$。**
...
int main()
{
    ...
    int dis_k = 0;
    // dis_k = k * (sz - k + 1)
    int _x = n - x + 1, _y = y; // take care of mapping
    for (int i = 1; i <= sz; i ++ )
    {
        k ++ ;
        dis_k = k * (sz - k + 1);
        char c = s[i];
        a[_x][_y] -= dis_k;
        if (c == 'N') _x = _x + dx[0], _y = _y + dy[0];
        else if (c == 'S') _x += dx[1], _y += dy[1];
        else if (c == 'W') _x += dx[2], _y += dy[2];
        else if (c == 'E') _x += dx[3], _y += dy[3];
    }
